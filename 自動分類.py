...
def split_dataset_balanced(
    images_path, labels_path, output_path, train_ratio=0.7, val_ratio=0.2, test_ratio=0.1, seed=42
):
    ...
    # 复制训练集
    print("\n正在复制训练集...")
    train_copy, _train_error = copy_files(train_files, "train")

    # 复制验证集
    print("正在复制验证集...")
    val_copy, _val_error = copy_files(val_files, "val")

    # 复制测试集
    print("正在复制测试集...")
    test_copy, _test_error = copy_files(test_files, "test")
    ...
    for split in ["train", "val", "test"]:
        split_images = len(
            [
                f
                for f in os.listdir(os.path.join(output_path, "images", split))
                if f.lower().endswith((".jpg", ".png", ".jpeg"))
            ]
        )
        split_labels = len([f for f in os.listdir(os.path.join(output_path, "labels", split)) if f.endswith(".txt")])

        print(
            f"{split}集 - 图片: {split_images}, 标签: {split_labels}, {'匹配' if split_images == split_labels else '不匹配!'}"
        )
